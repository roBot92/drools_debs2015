...
public class CustomTreeMultiset{
	...
	private TreeMultiset<BigDecimal> multiSet = TreeMultiset.create((n1, n2) -> Double.compare(n1.doubleValue(), n2.doubleValue()));
	...
	public boolean addAll(Collection<? extends BigDecimal> elementsToAdd) {
		...
		for(BigDecimal number : elementsToAdd){
			if(multiSet.add(number)){
				result = true;
			}
		}
		return result;
