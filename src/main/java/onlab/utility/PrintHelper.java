	...
	private static long previousTime = 0;
		...
		public static String handlePrintActions(final ToplistSetInterface toplist, final int runningMode,
			...
			if (previousTime == 0) {
				...
				if (runningMode == DebsMain.TIME_MEASURING_MODE) {
			...
			if (!toplistStringWithoutDelay.equals(previousToplistWithoutDelay)) {
				if (runningMode == DebsMain.TIME_MEASURING_MODE || runningMode == DebsMain.MEMORY_MEASURING_MODE) {
			...
			if ((runningMode == DebsMain.TIME_MEASURING_MODE || runningMode == DebsMain.MEMORY_MEASURING_MODE)
				...
				if (runningMode == DebsMain.TIME_MEASURING_MODE) {
					...
					System.gc();
					try {
						Thread.sleep(DebsMain.SLEEP_TIME_IN_MS);
